...
/** Get single worker by IPUC code (private worker link). */
...
// Maps canonical (camelCase) field names to their Supabase column names.
// updateWorker uses this to send ONLY the fields that were actually passed
// in `patch` — never the whole row, and never `slug` (per spec, editing
// content must never change the worker's public URL).
...
export async function updateWorker(id, patch) {
  ...
  Object.keys(patch).forEach((key) => {
    ...
    if (!column) return; // ignore unknown fields (and slug/id, on purpose)
    cleanPatch[column] = key === "experience" ? Number(patch[key]) || 0 : patch[key];
  });
